...
router.get('/:politician_no',async (req,res)=>{ //기본 신상 정보 라우터
    ...
    try{
        ...
        var weekEndDay=moment().isoWeekday(6).format('YYYY-MM-DD');
        ...
        for (let i=0;i<4;i++){//4주 까지 가져옴
            ...
            weekEndDay=moment(weekDay).isoWeekday(6).format('YYYY-MM-DD');
            //weekEndDay=moment().isoWeekday(((i+1)*-7)+6).format('YYYY-MM-DD');
        }
