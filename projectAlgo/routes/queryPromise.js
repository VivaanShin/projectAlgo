...
exports.getBlackUserGrade=function getBlackUserGrade(connection){ //connection 하나를 전달 받아서 사용,동기형으로 tn_user_politician_grade를 모두 가져옴
    return new Promise((resolve,reject)=>{
        connection.query(`select * from tb_gradeinfo_record as gr, tb_user_info as ui, tb_politician_info as pi where gr.user_id=ui.user_id and ui.user_black = 1 and pi.politician_no=gr.politician_no`
        ,(err,userGrade)=>{
            if(err)
                reject(err);
            resolve(userGrade);
