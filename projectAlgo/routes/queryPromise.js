...
exports.updateAdminUserInfo=function updateAdminUserInfo(user,connection){ //connection 하나를 전달 받아서 사용,동기형으로 사용자 정보를 업데이트 어드민에서 회원정보 수정시 사용 
    return new Promise((resolve,reject)=>{
        connection.query(`update tb_user_info set user_email=?,user_phone=?,user_interest_check=?,user_black=?
        where user_id=?`,[user.user_email,user.user_phone,user.user_interest_check,user.user_black,user.user_id],
        (err,user)=>{
            if(err)
                reject(err);
            resolve(user); 
